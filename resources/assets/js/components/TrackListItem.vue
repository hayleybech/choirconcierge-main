	...
	<tr>
		<td class="col--play">
			<template v-if="isPlayable">
				<button v-if="isCurrent" class="btn btn-link disabled btn-sm" disabled>
					<i class="fas fa-fw fa-waveform"></i>
				</button>
				<button v-else v-on:click="$emit('play')" class="btn btn-link btn-sm">
		...
		<td class="col--download">
			<a :href="attachment.download_url" class="btn btn-link btn-sm" :download="attachment.filepath"
				><i class="fa fa-fw fa-download"></i
			></a>
		</td>
		<td class="col--delete" v-if="canUpdate">
			<delete-button
				:index="'attachment-' + attachment.id"
				:action="'/songs/' + song.id + '/attachments/' + attachment.id"
				class-name="btn-sm"
				:enable-padding="true"
			></delete-button>
		</td>
...
export default {
	...
	computed: {
		isPlayable() {
			return (
				this.attachment.category.title === 'Learning Tracks' ||
