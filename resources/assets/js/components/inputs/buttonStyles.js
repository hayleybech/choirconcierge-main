...
const buttonStyles = (variant = 'secondary', size = 'md', disabled, extra) => classNames(
    extra,
    'inline-flex justify-center items-center gap-x-1.5 border shadow-xs font-medium focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-purple-500',
    ...
    size === 'sm' && (variant !== 'clear' && variant !== 'danger-clear') ? 'py-1.5 px-3 text-sm rounded-sm' : '',
    size === 'sm' && (variant === 'clear' || variant === 'danger-clear') ? 'py-1.5 px-1.5 text-sm rounded-sm' : '',
    size === 'xs' ? 'py-1 px-1.5 text-xs rounded-xs' : '',
);
