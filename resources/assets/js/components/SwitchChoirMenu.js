...
const SwitchChoirMenu = ({ choirs: organisations, tenant, mobile = false }) => {
		...
		return (
			<>
				...
				>
					{tenant ? (
						<>
							<div className="grow flex justify-center p-1">
								{tenant.logo_url ? (
									<img src={tenant.logo_url} alt={tenant.name} className="max-h-12 w-auto" />
								) : (
									tenant.name
								)}
							</div>
						...
						<>
							<div className="grow text-center">Switch Choir</div>
									...
									<div className="divide-y divide-gray-200">
										{organisations.map(org => (
											<Link
												...
												className="block px-4 py-3 text-gray-700 hover:bg-gray-100 flex justify-center items-center"
											>
												{org.logo_url ? (
													<img
														src={org.logo_url}
	...
	return (
		<Menu as="div" className="relative inline-block text-left grow w-full">
			...
			>
								<Menu.Items className="absolute right-0 z-20 mt-2 w-full max-h-[80dvh] sm:max-h-[60dvh] overflow-y-scroll origin-top-left bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
								...
								{({ active }) => (
									<Link
										...
										className={classNames(
											...
											'block px-4 py-2 text-sm flex justify-center items-center'
										)}
									>
										{org.logo_url
