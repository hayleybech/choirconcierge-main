...
export class RiserFrame {
    ...
    private readonly num_rows: number;          // How many rows? We'll draw (num_rows + 1) arcs vertically.
    private readonly num_cols: number;          // How many rows? We'll draw (n_cols) arcs horizontally.
    ...
    constructor(selector: string, height: number, width: number, rows: number, cols: number, singers: number) {
        ...
        this.canvas_width = Number(width);
        this.num_rows = Number(rows);
        this.num_cols = Number(cols);
        this.num_singers = Number(singers);
    ...
    draw(): void
    {
        ...
        let spots = new RiserSpots(this.snap, this.origin, this.risers_start_radius, this.row_height_radius, this.total_width_deg, this.num_rows, false, this.num_singers);
        spots.draw();
    }
