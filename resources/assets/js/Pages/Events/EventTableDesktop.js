		...
		<Table pagination={<Pagination details={events} />}>
			<THead>
				<tr>
					...
					<TableHeading>
						<TableHeadingSort form={sortFilterForm} sort="type-title">
							Type
						</TableHeadingSort>
					</TableHeading>
					{showEnsemblesColumn && <TableHeading>Ensembles</TableHeading>}
					<TableHeading>
						<TableHeadingSort form={sortFilterForm} sort="start_date">
							Event Date
						</TableHeadingSort>
					</TableHeading>
			...
			<TBody>
				{events.data.map(event => (
					<TItemRow key={event.id} bulkEdit={bulkEdit} value={event.id}>
						...
						<TableCell className="max-w-[60ch] truncate">
							<Link
								href={route('events.show', { event })}
								className="text-sm font-medium text-purple-800 max-w-full"
							>
								{event.title}
								{event.is_repeating && (
