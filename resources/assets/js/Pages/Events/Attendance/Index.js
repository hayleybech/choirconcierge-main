        ...
        <>
            ...
            <PageHeader
                ...
                meta={[
                    <div className="text-gray-500">
                        ...
                        <p className="mb-2">The kiosk page allows singers to mark themselves off from a shared device,
                            while the individual check-in page allows singers to use their own device by scanning a QR
                            code. Both check-in pages automatically mark singers as late after the call time, or absent
                            20 minutes later.</p>
                        <p className="mb-2">If attendance was recorded during the event (either manually here or using
                            the kiosk), remaining singers will automatically be marked absent once the event ends.</p>
                    </div>,
                ]}
            />
            ...
            >
                <div className="w-full">
                    ...
                    <p className="mb-2">They can scan this QR code while logged in to gain temporary access to the
                        check-in page.</p>
                            ...
                            {[
                                {
                                    ...
                                    colour: 'emerald',
                                    icon: 'check',
                                    count: part.members.filter(attendance => attendance.response === 'present').length
                                },
                                {
                                    label: 'Late',
                                    colour: 'amber',
                                ...
                                },
                                {
                                    ...
                                    colour: 'red',
                                    icon: 'times',
                                    count: part.members.filter(attendance => attendance.response === 'absent' || attendance.response === 'absent_apology').length
                                },
                                {
                                    label: 'Unknown',
                                    colour: 'gray',
                                    icon: 'question',
                                    count: part.members.filter(attendance => attendance.response === 'unknown').length
                                },
                            ].map(({label, colour, icon, count}) => (
                                <div className="w-1/3 md:w-1/5 text-center flex flex-col items-center justify-between"
                                     key={label}>
                                    <div className="hidden md:block">
                                        <AttendanceTag label={label} icon={icon} colour={colour} size="md"
                        ...
                        <ul role="list" className="relative z-0 divide-y divide-gray-200">
                            {part.members.map((attendance) => (
                                <li key={attendance.member.id} className="bg-white">
                                    <div
                                        className="relative px-6 py-5 flex flex-col xl:flex-row items-stretch xl:items-center gap-y-3 sm:gap-x-3 hover:bg-gray-50 justify-between">
                                        <div className="flex space-x-2 shrink-0">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-sm">
                                                    <AttendanceTag label={attendance.label} icon={attendance.icon}
                                                                   colour={attendance.colour}/>
                                                    {!!attendance.updated_at && (
                                                        <DateTag label="Updated" date={attendance.updated_at}
                                                                 format="DATETIME_SHORT" className="text-gray-400"/>
                                                    )}
                                                    {attendance.response.includes('absent') && attendance.absent_reason && (
                                                        <div className="text-gray-500">Reason for
                                                            absence: {attendance.absent_reason}</div>
                                                    )}
                                                </p>
                                            </div>
                                        </div>
                                        <div
                                            className="shrink-0 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 items:stretch sm:items-end">
                                            {[
                                                {
                                                    response: 'present',
                                                    label: 'On Time',
                                                ...
                                                <Button
                                                    ...
                                                    data={{
                                                        response: response,
                                                        absent_reason: absentReasons[attendance.member.id]
                                                    }}
                                                    preserveScroll
                                                    size="sm"
                                                    variant={variant}
                                            ...
                                            <Button
                                                ...
                                                method="put"
                                                data={{
                                                    response: !!absentReasons[attendance.member.id] ? 'absent_apology' : 'absent',
                                                    absent_reason: absentReasons[attendance.member.id],
                                                }}
                                                preserveScroll
                                                size="sm"
